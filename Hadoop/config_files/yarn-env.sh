# Hadoop YARN (Yet Another Resource Negotiator) 框架的环境变量配置文件，主要用于设置 YARN 相关守护进程（如 ResourceManager 和 NodeManager）的运行时环境变量，尤其是它们的 JVM 内存选项。
# 推荐配置：YARN_RESOURCEMANAGER_HEAPSIZE（控制 ResourceManager 进程的 JVM 堆内存大小。ResourceManager 是 YARN 的核心组件，负责集群的资源分配和应用程序调度。适当的内存设置对其性能和稳定性至关重要。）、YARN_NODEMANAGER_HEAPSIZE（控制 NodeManager 进程的 JVM 堆内存大小。NodeManager 负责管理其节点上的计算资源和应用程序容器。）
